...
import { parseAPI, type Industry, type IndustryDeep, type IndustryOptions } from '../src/index.js';
...
it('looks up ranged sectors and preserves hierarchy, nulls and unknown fields', async () => {
 ...
 await parse.industry('54/11');
 expect(urls).toEqual(['https://api.parseapi.com/industry/31-33', 'https://api.parseapi.com/industry/54%2F11']);
 expectTypeOf<Industry['parent']>().toEqualTypeOf<string | null>();
 expectTypeOf<IndustryDeep['description']>().toEqualTypeOf<string | null>();
 expectTypeOf<'deep'>().toMatchTypeOf<keyof IndustryOptions>();
});
...
it('searches with encoded keywords and optional limits, preserving empty results', async () => {
 ...
 await parse.industry.search('plumbing');
 expect(urls).toEqual(['https://api.parseapi.com/industry?q=coffee+%26+tea&limit=5', 'https://api.parseapi.com/industry?q=plumbing']);
});
...
it('keeps honest code misses on the API error path without retrying 404', async () => {
 ...
 await expect(parse.industry('999999')).rejects.toMatchObject({ status: 404, code: 'not_found', requestId: 'req_naics' });
 expect(fetch).toHaveBeenCalledTimes(1);
});


it('preserves classification exclusions and search evidence without requiring new fields', async () => {
 const legacy: Industry = {"naics":"541511","name":"Custom Computer Programming Services","level":6,"parent":"54151","parent_name":"Computer Systems Design and Related Services","year":2022,"country":"US"};
 ...
 const search = await parse.industry.search('sofware');
