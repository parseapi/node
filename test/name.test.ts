...
it('keeps name membership independent of gender and sends optional context', async () => {
	const urls: string[] = [];
	const parse = parseAPI('test_key', { fetch: async input => {
		urls.push(String(input));
		return new Response('{"name":"王","valid":true,"deep":{"known":true,"gender":null},"future":true}');
	} });
	...
	expect(result).toMatchObject({ deep: { known: true, gender: null }, future: true });
